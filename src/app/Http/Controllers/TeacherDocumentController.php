...
class TeacherDocumentController extends Controller
{
    ...
    {  
        $doc['getData'] = Document::getDocument($request); // Pass the Request object  
        $doc['header_title'] = "Document List";  
        return view('teacher.document.list', $doc);  
    }
    ...
    {  
        $doc['getData'] = Document::getDocument($request); // Pass the Request object  
        $doc['header_title'] = "Document List";  
        return view('teacher.dashbroad', $doc);  
    }

    public function document_add()  
    {  
        $doc['header_title'] = "Add New Document";  
        return view('teacher.document.add', $doc);  
    } 
...
{  
    ...
    try {  
        ...
        return redirect('teacher/document/list')->with('success', "Document successfully added");  
    } catch (\Exception $e) {  
    ...
    {  
       ...
       $out = Output::getOutID($doc->output_id);  
        ...
        return redirect('teacher/document/list')->with('success', "Document successfully updated");  
    }
    ...
    {  
        ...
        if ($docId) {  
            // Assuming you have similar relationships in other tables  
            Information::where('doc_id', '=', $id)->delete();  
            Subject::where('doc_id', '=', $id)->delete();  
            Requirement::where('doc_id', '=', $id)->delete();  
            Reference::where('doc_id', '=', $id)->delete();  
            Output::where('doc_id', '=', $id)->delete();  
            Content::where('doc_id', '=', $id)->delete();  
            Evaluation::where('doc_id', '=', $id)->delete();  
            Regulation::where('doc_id', '=', $id)->delete();  
        }  
        
        if ($informationId) {  
            Document::where('information_id', $informationId)->delete();  
            Information::where('id', $informationId)->delete();  
        }  
        ...
        if ($requirementId) {  
            ...
            Requirement::where('id', $requirementId)->delete();  
        }  

        if ($referenceId) {  
            Document::where('reference_id', $referenceId)->delete();  
            Reference::where('id', $referenceId)->delete();  
        }  

        if ($outputId) {  
            Document::where('output_id', $outputId)->delete();  
            Output::where('id', $outputId)->delete();  
        }  
        ...
        if ($evaluationId) {  
            ...
            Evaluation::where('id', $evaluationId)->delete();  
        }  

        if ($regulationId) {  
            Document::where('regulation_id', $regulationId)->delete();  
            Regulation::where('id', $regulationId)->delete();  
        }  


        // Now delete all records in the documents table with the same doc_id  
        if ($docId) {  
            ...
            return redirect('teacher/document/list')->with('success', "Document and related entries successfully deleted");  
        }          
...
{  
    ...
    if (!empty($data['getSub'])) {  
        ...
        return view('teacher.document.view', $data);  
    } else {  
        abort(404);  
    }  
}
...
{  
        ...
        $data['getRegulation'] = Regulation::getRegulationID($data['getData']->regulation_id); 
            ...
            return view('teacher.document.print', $data);  
