    ...
    <meta charset="UTF-8">  
        ...
        <style>  
            ...
            .highlight {  
                background-color: yellow; /* Highlight color for differences */  
            } 
        </style>  
            ...
            <div class="card-header">
                ...
                <div class="form-group row justify-content-center">   
                    <div class="col-sm-12 text-center">  
                        ...
                        <h1>  
                            <input type="text" class="highlight" name="doc_name" 
                            value="{{ old('doc_name', $getData->doc_name) }}" 
                            style="display: inline-block; width: 100%; border: none; background: transparent; font-weight: bold; font-size: 32px; text-align: center;" readonly>   
                        </h1>  
                        <h3 style="font-weight: bold; font-size: 24px; text-align: right;">  
                            MSHP: <input type="number" class="highlight" name="mhp" 
                            value="{{ old('mhp', $getData->mhp) }}" 
                            style="display: inline-block; width: 50%; border: none; background: transparent; font-weight: bold; font-size: 24px;" readonly>  
                        </h3>  
                    </div>  
                </div>  
...
<script>  
    // Assuming mainData is fetched or defined beforehand  
    const mainData = {  
        id: {{ $getData->id }},  
        doc_name: {{ $getData->doc_name }},  
        mhp: {{ $getData->mhp }},  
        doc_id: null,  
    };   
    ...
    const compareData = {  
        ...
        doc_name: {{ $getData->doc_name }},  
        mhp: {{ $getData->mhp }},  
        doc_id: mainData.id, // Set doc_id to be the same as mainData.id  
    };
    // Check if the compareData doc_id is the same as the mainData id  
    if (compareData.doc_id === mainData.id) {  
        // Highlight inputs if the values differ  
        if (compareData.doc_name !== mainData.doc_name) {  
            document.querySelector('input[name="doc_name"].compareData').classList.add('highlight');  
        }  
        if (compareData.mhp !== mainData.mhp) {  
            document.querySelector('input[name="mhp"].compareData').classList.add('highlight');  
        }  
    }  
</script>
                    ...
                    <table class="table table-bordered">  
                        <tbody>  
                            <tr style="font-weight: bold; border: 2px solid black;">  
                                ...
                                <td style="width: 80%; border: 2px solid black;"><input class="col-md-2 form-control" type="text" name="prerequisite" readonly value="{{ old('prerequisite', $getRequi->prerequisite ?? '') }}"></td>   
                            </tr>   

                            <tr style="font-weight: bold; border: 2px solid black;">  
                                <td style="width: 20%; text-align: LEFT; border: 2px solid black;"><label for="HPSH">Học phần song hành</label></td>  
                                <td style="width: 80%; border: 2px solid black;"><input class="col-md-2 form-control" type="text" name="corequisite" readonly value="{{ old('corequisite', $getRequi->corequisite ?? '') }}"></td>    
                            </tr>   
